...
struct skepu2_userfunction_mapReduce_dist
{
constexpr static size_t totalArity = 3;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<Point, Point, int>;
...
static inline SKEPU_ATTRIBUTE_FORCE_INLINE float OMP(Point p1, Point p2, int dim)
{
  int i;
  float result=0.0;
  for (i=0;i<dim;i++)
...
static inline SKEPU_ATTRIBUTE_FORCE_INLINE float CPU(Point p1, Point p2, int dim)
{
  int i;
  float result=0.0;
  for (i=0;i<dim;i++)
...
struct skepu2_userfunction_map2_dist
{
constexpr static size_t totalArity = 3;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<>;
...
struct skepu2_userfunction_map3_dist
{
constexpr static size_t totalArity = 3;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<>;
...
struct skepu2_userfunction_map_dist
{
constexpr static size_t totalArity = 3;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<>;
...
struct skepu2_userfunction_map_pspeedyMapFunction
{
constexpr static size_t totalArity = 3;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<Point>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<Point, int>;
constexpr static skepu2::AccessMode anyAccessMode[] = {
};
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE Point OMP(Point p, Point x, int dim)
{
    float distance = skepu2_userfunction_map_dist::OMP(p, x, dim);
    p.assign = 0;
    p.cost = distance * p.weight;
    return p;
}
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE Point CPU(Point p, Point x, int dim)
{
    float distance = skepu2_userfunction_map_dist::CPU(p, x, dim);
    p.assign = 0;
    p.cost = distance * p.weight;
    return p;
}
...
struct skepu2_userfunction_map2_pspeedyMapFunction2
{
constexpr static size_t totalArity = 4;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<Point>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<int, Point, int>;
constexpr static skepu2::AccessMode anyAccessMode[] = {
};
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE Point OMP(Point p, int dim, Point x, int i)
{
	float distance = skepu2_userfunction_map2_dist::OMP(x, p, dim);
	if(distance*p.weight < p.cost){
		p.cost = distance * p.weight;
		p.assign = i;
	}
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE Point CPU(Point p, int dim, Point x, int i)
{
	float distance = skepu2_userfunction_map2_dist::CPU(x, p, dim);
	if(distance*p.weight < p.cost){
		p.cost = distance * p.weight;
		p.assign = i;
	}
...
float pspeedy(Points *points, float z, long *kcenter)
{
  ...
  skepu2::backend::Map<1, skepu2_userfunction_map_pspeedyMapFunction, bool, void> map(false);
  ...
  skepu2::backend::Map<1, skepu2_userfunction_map2_pspeedyMapFunction2, bool, void> map2(false);
...
struct skepu2_userfunction_map_pgainMapFunction
{
constexpr static size_t totalArity = 6;
constexpr static bool indexed = 1;
using ElwiseArgs = std::tuple<>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<double *, int, long, Points *, long>;
constexpr static skepu2::AccessMode anyAccessMode[] = {
};
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double OMP(skepu2::Index1D index, double* work_mem, int stride, long bsize, Points* points, long x)
{
  long k1 = bsize*index.i;
  long k2 = k1 + bsize;
  if (index.i == nproc-1) k2 = points->num;
  ...
  double* lower = &work_mem[index.i*stride];
  for (int i = k1; i < k2; i++ ) {
	  float x_cost = skepu2_userfunction_map_dist::OMP(points->p[i], points->p[x], points->dim)
		* points->p[i].weight;
	  float current_cost = points->p[i].cost;
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double CPU(skepu2::Index1D index, double* work_mem, int stride, long bsize, Points* points, long x)
{
  long k1 = bsize*index.i;
  long k2 = k1 + bsize;
  if (index.i == nproc-1) k2 = points->num;
  ...
  double* lower = &work_mem[index.i*stride];
  for (int i = k1; i < k2; i++ ) {
	  float x_cost = skepu2_userfunction_map_dist::CPU(points->p[i], points->p[x], points->dim)
		* points->p[i].weight;
	  float current_cost = points->p[i].cost;
...
struct skepu2_userfunction_map2_pgainMapFunction2
{
constexpr static size_t totalArity = 10;
constexpr static bool indexed = 1;
using ElwiseArgs = std::tuple<double>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<double *, int, double *, long, long, double, long, int>;
constexpr static skepu2::AccessMode anyAccessMode[] = {
};
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double OMP(skepu2::Index1D index, double cost_of_opening_x, double* work_mem, int stride, double* gl_lower, long bsize, long num, double z, long x, int K)
{
	 int number_of_centers_to_close = 0;
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double CPU(skepu2::Index1D index, double cost_of_opening_x, double* work_mem, int stride, double* gl_lower, long bsize, long num, double z, long x, int K)
{
	 int number_of_centers_to_close = 0;
...
struct skepu2_userfunction_map3_pgainMapFunction3
{
constexpr static size_t totalArity = 5;
constexpr static bool indexed = 1;
using ElwiseArgs = std::tuple<>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<long, double *, Points *, long>;
constexpr static skepu2::AccessMode anyAccessMode[] = {
};
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double OMP(skepu2::Index1D index, long bsize, double* gl_lower, Points* points, long x)
{
  long k1 = bsize*index.i;
  long k2 = k1 + bsize;
  if (index.i == nproc-1) k2 = points->num;
  for ( int i = k1; i < k2; i++ ) {
	  ...
	  if ( switch_membership[i] || close_center ) {
		  ...
		  // or i is closer to x than to its current median
		  points->p[i].cost = points->p[i].weight *
				  skepu2_userfunction_map3_dist::OMP(points->p[i], points->p[x], points->dim);
		  points->p[i].assign = x;
	  }
  }
  for( int i = k1; i < k2; i++ ) {
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double CPU(skepu2::Index1D index, long bsize, double* gl_lower, Points* points, long x)
{
  long k1 = bsize*index.i;
  long k2 = k1 + bsize;
  if (index.i == nproc-1) k2 = points->num;
  for ( int i = k1; i < k2; i++ ) {
	  ...
	  if ( switch_membership[i] || close_center ) {
		  ...
		  // or i is closer to x than to its current median
		  points->p[i].cost = points->p[i].weight *
				  skepu2_userfunction_map3_dist::CPU(points->p[i], points->p[x], points->dim);
		  points->p[i].assign = x;
	  }
  }
  for( int i = k1; i < k2; i++ ) {
...
double pgain(long x, Points *points, double z, long int *numcenters)
{
  ...
  */
  ...
  skepu2::backend::Map<0, skepu2_userfunction_map_pgainMapFunction, bool, void> map(false);
  auto spec = skepu2::BackendSpec{skepu2::Backend::Type::OpenMP};
  spec.setCPUThreads(nproc);
  map.setBackend(spec);
  map(cost_of_opening_x, work_mem, stride, bsize, points, x);

  skepu2::backend::Map<1, skepu2_userfunction_map2_pgainMapFunction2, bool, void> map2(false);
  map2.setBackend(spec);
  ...
  if ( gl_cost_of_opening_x < 0 ) {
      ...
      skepu2::backend::Map<0, skepu2_userfunction_map3_pgainMapFunction3, bool, void> map3(false);
...
struct skepu2_userfunction_mapReduce_sum
{
constexpr static size_t totalArity = 2;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<double, double>;
using ContainerArgs = std::tuple<>;
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double OMP(double a, double b)
{
    return a+b;
}
#undef SKEPU_USING_BACKEND_OMP
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double CPU(double a, double b)
{
    return a+b;
}
#undef SKEPU_USING_BACKEND_CPU
...
struct skepu2_userfunction_mapReduce_pkmedianMapFunction
{
constexpr static size_t totalArity = 3;
constexpr static bool indexed = 0;
using ElwiseArgs = std::tuple<Point>;
using ContainerArgs = std::tuple<>;
using UniformArgs = std::tuple<Point, long>;
constexpr static skepu2::AccessMode anyAccessMode[] = {
};
...
#define VARIANT_OPENMP(block) block
#define VARIANT_CUDA(block)
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double OMP(Point p, Point x, long dim)
{
    return skepu2_userfunction_mapReduce_dist::OMP(p, x, dim)*p.weight;
}
#undef SKEPU_USING_BACKEND_OMP
...
#define VARIANT_OPENMP(block)
#define VARIANT_CUDA(block) block
static inline SKEPU_ATTRIBUTE_FORCE_INLINE double CPU(Point p, Point x, long dim)
{
    return skepu2_userfunction_mapReduce_dist::CPU(p, x, dim)*p.weight;
}
#undef SKEPU_USING_BACKEND_CPU
};
...
float pkmedian(Points *points, long kmin, long kmax, long* kfinal)
{
  ...
  skepu2::backend::MapReduce<1, skepu2_userfunction_mapReduce_pkmedianMapFunction, skepu2_userfunction_mapReduce_sum, bool, bool, void> mapReduce(false, false);
  auto spec = skepu2::BackendSpec{skepu2::Backend::Type::OpenMP};
  spec.setCPUThreads(nproc);
