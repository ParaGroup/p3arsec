...
class POFEmitter: public ff::ff_node{
private:
    ff::ff_loadbalancer* _lb;
    size_t _nw;
public:
    POFEmitter(ff::ff_loadbalancer* lb, size_t nw):_lb(lb), _nw(nw){;}
    ...
    void* svc(void* task){
        ...
        if(isLast){
            ...
            for(size_t i = 1; i < _nw; i++){
                recv_buf = (ringbuffer_t*) malloc(sizeof(ringbuffer_t));
                ringbuffer_init(recv_buf, ANCHOR_DATA_PER_INSERT);
                ringbuffer_setLast(recv_buf);
                while(!_lb->ff_send_out_to((void*) recv_buf, i)){;}
            }
            return GO_ON;
        }else{
            return task;
...
static void runPipeOfFarms(config_t * conf, struct thread_args* data_process_args,
        struct thread_args* send_block_args, stats_t **threads_anchor_rv,
        stats_t **threads_chunk_rv, stats_t **threads_compress_rv){
    ff::ff_pipeline p;
    ff::ff_farm<> fraFarm, dedFarm, cmpFarm;
    std::vector<ff::ff_node*> fraW, dedW, cmpW;
    for(size_t i = 0; i < conf->nthreads; i++){
        fraW.push_back(new FragmentRefine(true));
        dedW.push_back(new Deduplicate(true));
        cmpW.push_back(new Compress(true));
    }
    fraFarm.add_emitter(new Fragment(data_process_args, conf->nthreads, fraFarm.getlb()));
    fraFarm.add_workers(fraW);
    fraFarm.add_collector(new POFCollector(fraFarm.getgt(), conf->nthreads));
	fraFarm.cleanup_all();

    dedFarm.add_emitter(new POFEmitter(dedFarm.getlb(), conf->nthreads));
    dedFarm.add_workers(dedW);
    dedFarm.add_collector(new POFCollector(dedFarm.getgt(), conf->nthreads));
	dedFarm.cleanup_all();

    cmpFarm.add_emitter(new POFEmitter(cmpFarm.getlb(), conf->nthreads));
    cmpFarm.add_workers(cmpW);
    cmpFarm.add_collector(new Reorder(send_block_args, conf->nthreads));
	cmpFarm.cleanup_all();

#ifdef ENABLE_FF_ONDEMAND
    fraFarm.set_scheduling_ondemand();
    dedFarm.set_scheduling_ondemand();
    cmpFarm.set_scheduling_ondemand();
#endif
...
void EncodeFF(config_t * _conf) {
  ...
  stats_t *threads_compress_rv[nthreadsstats];
    ...
    __parsec_roi_begin();
#endif
     runPipeOfFarms(conf, &data_process_args, &send_block_args,
