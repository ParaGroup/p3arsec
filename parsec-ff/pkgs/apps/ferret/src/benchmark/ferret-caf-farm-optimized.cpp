...
struct load_data file_helper (const std::string& filename)
{
	int r;
	struct load_data data;
	bzero(&data, sizeof(data));

	data.name = strdup(filename.c_str());

	r = image_read_rgb_hsv(filename.c_str(), &data.width, &data.height, &data.RGB, &data.HSV);
	assert(r == 0);
	return data;
}

struct seg_data segment(const std::string& filename){
	struct load_data task = file_helper(filename);
...
/* ------ The Stages ------ */
class CollapsedPipeline: public caf::event_based_actor {
private:
	caf::actor next_;

public:
	CollapsedPipeline(caf::actor_config& cfg, caf::actor next) :
	...
	caf::behavior make_behavior() override {
		return {
			[=](const std::string& file_name) {
				struct rank_data res = rank(vec(extract(segment(file_name))));
				send(next_, std::move(res));
			}
		};
	}
};
...
public:
  Load(caf::actor_config& cfg, caf::actor next) : caf::event_based_actor(cfg), next_(next) {
    path[0] = 0;
  }
...
int Load::scan_dir (const char *dir, char *head)
{
	...
	if (S_ISREG(st.st_mode)){
		send(next_, move(std::string(path)));
		cnt_enqueue++;
